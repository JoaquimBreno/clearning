...
int EhArrayOrdenado(int naMosca[]){
    int nMaior=0, i, crescente=0, decrescente=0;
    for(i=0; i<10; i++){
        if(naMosca[i]>=nMaior){
            nMaior=naMosca[1];
            crescente++;
    ...
    if(crescente==10 || decrescente==10){
        return 1;
    }
    else{
        return 0;
...
int main(void){
    ...
    if(EhArrayOrdenado(naMosca)){
        printf("Esse array é ordenado.\n");
    }
    else{
        printf("Esse array não é ordenado.\n");
    }
    return 0;
}